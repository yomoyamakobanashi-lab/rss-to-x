...
DEDUPE_STRATEGY = os.getenv("DEDUPE_STRATEGY", "skip").lower()  # "skip" or "tweak"
...
def norm_title(s: str) -> str:
    return (s or "").lower().translate(_PUNC)

def title_sim(a: str, b: str) -> float:
    return difflib.SequenceMatcher(a=norm_title(a), b=norm_title(b)).ratio()

def find_apple_episode_url(entry, collection_id: Optional[str], country="JP") -> Optional[str]:
...
def pick_best_link_for_podcast(entry, feed: Dict[str, Any]) -> Optional[str]:
    ap = find_apple_episode_url(entry, feed.get("apple_collection_id"))
...
def render_body_without_link(template: str, title: str, program: str) -> str:
    body = template
    for k in ("{title}","{タイトル}"): body = body.replace(k, title)
    for k in ("{program}","{番組名}"): body = body.replace(k, program)
    for k in ["{link}","{URL}","{Url}","{url}","{エピソードURL}"]: body = body.replace(k, "").rstrip()
...
def compose_text(template: str, title: str, program: str, link: str,
    ...
    if x_length(cand) <= limit: return cand
    ...
    if x_length(cand_progless) <= limit and pref: return cand_progless
    cand_prefix = (pref + ("\n" + normalize_link(link) if link else "")).strip()
    if x_length(cand_prefix) <= limit and pref: return cand_prefix
    return normalize_link(link)

def whoami() -> Optional[str]:
    try:
        auth = OAuth1(os.getenv("X_API_KEY"), os.getenv("X_API_SECRET"),
                      os.getenv("X_ACCESS_TOKEN"), os.getenv("X_ACCESS_SECRET"))
        r = requests.get("https://api.twitter.com/2/users/me",
                         auth=auth, headers={"User-Agent": USER_AGENT}, timeout=HTTP_TIMEOUT)
        if r.status_code >= 300:
            logger.info("whoami failed: %s %s", r.status_code, r.text[:200]); return None
        u = r.json().get("data", {})
        logger.info("Posting as: @%s (id=%s)", u.get("username"), u.get("id"))
        return u.get("username")
    except Exception as e:
        logger.info("whoami exception: %s", e); return None

def post_raw_to_x(text: str) -> Tuple[int, str, dict]:
    """低レベル投稿（JSON返す）。"""
    if DRY_RUN:
        logger.info("[DRY_RUN] Tweet would be:\n%s", text)
        return 200, '{"dry_run": true}', {"data": {"id": "dryrun"}}
...
def post_to_x_with_dedupe(text: str) -> Tuple[bool, Optional[str], str]:
    """
    True/ tweet_id/ reason
    - 201/200 + data.id → True
    - 403 duplicate:
        - DEDUPE_STRATEGY=skip → True（tweet_id=None, reason='duplicate-skip'）
        - DEDUPE_STRATEGY=tweak → 目に見えない微調整を入れて再投稿
    """
    status, body, j = post_raw_to_x(text)
    if status in (200, 201) and isinstance(j, dict) and "data" in j and "id" in j["data"]:
        return True, str(j["data"]["id"]), "ok"

    # Duplicate判定（Xの403）
    if status == 403 and isinstance(j, dict) and str(j.get("title")).lower() == "forbidden" \
       and "duplicate" in (str(j.get("detail")) or "").lower():
        if DEDUPE_STRATEGY == "skip":
            logger.info("Duplicate detected -> skipping post but marking state.")
            return True, None, "duplicate-skip"
        else:
            # 微調整（不可視のWord Joinerを末尾に入れる）
            tweaked = text + "\u2060"
            logger.info("Duplicate detected -> retry with tweak.")
            status2, body2, j2 = post_raw_to_x(tweaked)
            if status2 in (200, 201) and isinstance(j2, dict) and "data" in j2 and "id" in j2["data"]:
                return True, str(j2["data"]["id"]), "tweaked"
            return False, None, f"duplicate-retry-failed status={status2} body={body2[:200]}"

    return False, None, f"post-failed status={status} body={body[:200]}"
...
def main() -> None:
    ...
    try:
        ...
        logger.error("feeds.json 読み込み失敗: %s", e); sys.exit(2)
    ...
    if not isinstance(feeds, list):
        logger.error("feeds.json の形式が不正（feeds が list ではない）"); sys.exit(2)
    ...
    for feed in feeds:
        ...
        for entry in entries[:CHECK_ITEMS]:
            ...
            if minutes_since(entry) < FRESH_WAIT_MIN:
                logger.info("[WAIT] fresh episode（%s）", getattr(entry,"title","")); continue
    ...
    if not candidates:
        logger.info("[INFO] 投稿候補なし（今回の実行）"); return

    chosen = sorted(candidates, key=lambda c: -c["ts"])[0]
    logger.info("Posting text:\n%s", chosen["text"])

    ok, tweet_id, reason = post_to_x_with_dedupe(chosen["text"])
    if ok:
        state[chosen["uid"]] = int(time.time())
        save_state(state)
        logger.info("[OK] handled tweet (%s)%s",
                    reason, f" id={tweet_id}" if tweet_id else "")
        return

    logger.error("[NG] %s", reason)
    sys.exit(4)
